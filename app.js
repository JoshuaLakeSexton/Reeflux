...
/*
  ...
  - Fractal Pool advanced engine (WebGL -> CPU fallback)
*/
...
// optional global (useful for inline scripts)
...
function setupFractalPool() {
  ...
  // If this page is still using the old fractal UI (legacy), do nothing (prevents conflicts).
  // New fractal page has frPreset + frType etc.
  if (!$("frPreset") || !$("frType")) return;
  ...
  function tryInitWebGL() {
    const gl =
      canvas.getContext("webgl", {
        ...
        preserveDrawingBuffer: true, // helps toDataURL for PNG export
  ...
  function cpuRender(ctx, w, h) {
    ...
    const paletteFn = (t) => {
      ...
      switch(state.palette){
        case "nebula":
          r = 0.5+0.5*Math.cos(6.283*(t+0.05)+2.0);
          g = 0.5+0.5*Math.cos(6.283*(t+0.18)+1.0);
          b = 0.5+0.5*Math.cos(6.283*(t+0.35)+0.2);
          break;
        case "coral":
          r = 0.55+0.45*Math.cos(6.283*(t+0.0)+0.0);
          g = 0.55+0.45*Math.cos(6.283*(t+0.12)+2.0);
          b = 0.55+0.45*Math.cos(6.283*(t+0.28)+3.5);
          r = (r*0.8+0.2*1.0); g = (g*0.8+0.2*0.55); b = (b*0.8+0.2*0.35);
          break;
        case "mono":
          r=g=b = t<0.5 ? (t*1.8) : (0.2 + (t-0.5)*1.6);
          break;
        case "ice":
          r = 0.55+0.45*Math.cos(6.283*(t+0.0)+2.6);
          g = 0.55+0.45*Math.cos(6.283*(t+0.18)+3.6);
          b = 0.55+0.45*Math.cos(6.283*(t+0.33)+4.2);
          r = (r*0.75+0.25*0.55); g = (g*0.75+0.25*0.85); b = (b*0.75+0.25*1.0);
          break;
        case "gold":
          r = 0.12 + 1.2*Math.pow(t,0.6);
          g = 0.12 + 0.85*Math.pow(t,0.6);
          b = 0.12 + 0.35*Math.pow(t,0.6);
          break;
        default:
          r = 0.55+0.45*Math.cos(6.283*(t+0.0)+0.2);
          g = 0.55+0.45*Math.cos(6.283*(t+0.25)+1.6);
          b = 0.55+0.45*Math.cos(6.283*(t+0.5)+2.2);
      }
  ...
  function ensureContext() {
    ...
    if (want !== "cpu") {
      if (!state._gl) state._gl = tryInitWebGL();
      if (state._gl) {
        state._ctx2d = null;
        state._using = "webgl";
      ...
      if (want === "webgl") setBadge("Renderer: WebGL (unavailable)");
    }

    if (!state._ctx2d) state._ctx2d = canvas.getContext("2d", { alpha: false, desynchronized: true });
    state._gl = null;
    state._using = "cpu";
  ...
  function renderFrame() {
    ...
    if (state._using === "webgl" && state._gl) {
      const { gl, uni } = state._gl;
      gl.viewport(0, 0, w, h);
    ...
    if (state._ctx2d) cpuRender(state._ctx2d, w, h);
  }
  ...
  el.iter?.addEventListener("input", () => {
    state.iterations = parseInt(el.iter.value, 10);
    if (el.iterVal) el.iterVal.textContent = String(state.iterations);
    requestRender();
  });

  el.escape?.addEventListener("input", () => {
    state.escape = parseInt(el.escape.value, 10);
    if (el.escapeVal) el.escapeVal.textContent = String(state.escape);
    requestRender();
  });

  el.power?.addEventListener("input", () => {
    state.power = parseInt(el.power.value, 10);
    if (el.powerVal) el.powerVal.textContent = String(state.power);
    requestRender();
  });

  el.juliaRe?.addEventListener("input", () => {
    state.julia.re = parseFloat(el.juliaRe.value);
    if (el.juliaReVal) el.juliaReVal.textContent = state.julia.re.toFixed(3);
    requestRender();
  });

  el.juliaIm?.addEventListener("input", () => {
    state.julia.im = parseFloat(el.juliaIm.value);
    if (el.juliaImVal) el.juliaImVal.textContent = state.julia.im.toFixed(3);
    requestRender();
  });

  el.palette?.addEventListener("change", () => {
    state.palette = el.palette.value;
    requestRender(true);
  });

  el.cycle?.addEventListener("input", () => {
    state.cycle = parseFloat(el.cycle.value);
    if (el.cycleVal) el.cycleVal.textContent = state.cycle.toFixed(2);
  });

  el.gamma?.addEventListener("input", () => {
    state.gamma = parseFloat(el.gamma.value);
    if (el.gammaVal) el.gammaVal.textContent = state.gamma.toFixed(2);
    requestRender(true);
  });

  el.contrast?.addEventListener("input", () => {
    state.contrast = parseFloat(el.contrast.value);
    if (el.contrastVal) el.contrastVal.textContent = state.contrast.toFixed(2);
    requestRender(true);
  });

  el.quality?.addEventListener("change", () => {
    state.quality = parseFloat(el.quality.value);
    requestRender(true);
  });

  el.renderer?.addEventListener("change", () => {
    state.renderer = el.renderer.value;
    state._gl = null;
    state._ctx2d = null;
    requestRender(true);
  });
  ...
  el.copy?.addEventListener("click", async () => {
    const payload = JSON.stringify({
      ...
      cx: state.cx, cy: state.cy, zoom: state.zoom,
    });
    try {
      await navigator.clipboard.writeText(payload);
      showToast("Copied settings.");
    } catch {
      prompt("Copy settings JSON:", payload);
    }
  });
  ...
  el.paste?.addEventListener("click", async () => {
    ...
    try {
      ...
      txt = prompt("Paste settings JSON:") || "";
    }
    if (!txt) return;
    try {
      const obj = JSON.parse(txt);
      Object.assign(state, {
        renderer: obj.renderer ?? state.renderer,
        type: obj.type ?? state.type,
        iterations: obj.iterations ?? state.iterations,
        escape: obj.escape ?? state.escape,
        power: obj.power ?? state.power,
        palette: obj.palette ?? state.palette,
        cycle: obj.cycle ?? state.cycle,
        gamma: obj.gamma ?? state.gamma,
        contrast: obj.contrast ?? state.contrast,
        quality: obj.quality ?? state.quality,
        cx: obj.cx ?? state.cx,
        cy: obj.cy ?? state.cy,
        zoom: obj.zoom ?? state.zoom,
      });
      if (obj.julia) state.julia = { ...state.julia, ...obj.julia };
      syncUI();
      requestRender(true);
      showToast("Applied settings.");
    } catch (e) {
      console.error(e);
  ...
  el.png?.addEventListener("click", () => {
    try {
      // Render once before export to ensure buffer has content
      renderFrame();
      const link = document.createElement("a");
      link.download = `reeflux-fractal-${Date.now()}.png`;
